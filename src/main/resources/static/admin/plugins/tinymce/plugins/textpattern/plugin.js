...
var textpattern = (function () {
  ...
  var $_2p9qyiq3jc7tmei6 = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_efc2qdqbjc7tmej0.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_efc2qdqbjc7tmej0.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_efc2qdqbjc7tmej0.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_bqe4xwq8jc7tmeiq.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_bqe4xwq8jc7tmeiq.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_bqe4xwq8jc7tmeiq.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_80c9yqq7jc7tmeid.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_80c9yqq7jc7tmeid.checkKeyCode(keyCodes, e)) {
        $_80c9yqq7jc7tmeid.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_80c9yqq7jc7tmeid.checkCharCode(charCodes, e)) {
  ...
  var $_atgo5fq4jc7tmei9 = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_2p9qyiq3jc7tmei6.getPatterns(editor.settings));
    $_atgo5fq4jc7tmei9.setup(editor, patternsState);
    return $_gu2h1q2jc7tmei5.get(patternsState);
  });
  var Plugin = function () {
  };
